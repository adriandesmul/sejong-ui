...
class SchoolSelect extends React.Component {
  ...
  selectSchool(school_id) {
    const school = this.state.schools.find(item => item.school_id === school_id)
    if (school) { this.props.onUpdate(school) }
  }

  getSchools(country, state) {
    API.get('/school/find/school_state/' + state, (error, data) => {
  ...
  render() {
    return (
      <div>
        ...
        <div className="scs-module-element">
          <label>Country: </label>
          <ButtonOptions options={constants.countries}
        ...
        <div className="scs-module-element">
          <label>State: </label>
          <DropdownOptions options={constants.states[this.state.country]}
                           onUpdate={this.selectState}
                           value={this.state.state} />
        </div>
        }
        {this.state.schools &&
        <SelectionTable options={this.state.schoolsTable}
                        labels={['School', 'City']}
